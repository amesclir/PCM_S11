...
El formato de las areas es un formato PHYLIP con algunas modificaciones. Se indica el número de species en la filogenia y luego el número de áreas (nombres entre parántesis). Y luego tenemos una columna con los nombres de las especies tal y como aparecen en la filogenia y tantas columnas de 1 y 0 como áreas se hayan indicado, dodificando la presencia y ausencia de cada una de las especies.
...
Lo que hacemos es crear una lista con todos los argumentos necesarios para correr el análisis. Esta lista se crea con las condiciones por defecto (modelo DEC). Y lo podemos modificar los diferentes parámetros y argumentos según nuestro antojo, es decir, según los parámetros y condiciones que queremos ajustar en nuestro modelo biogeográfico (modelo DEC+J, DIVA, DIVA+J, etc., conexiones entre áreas, con o sin estratificar, parametros x y w, etc.).
...
En los últimos años se ha incrementado el número de parámetros, modelos y opciones disponibles en BioGeoBEARS. Estas son algunas referencias claves (Van Dam & Matzke, 2016; Dupin et al., 2016)
